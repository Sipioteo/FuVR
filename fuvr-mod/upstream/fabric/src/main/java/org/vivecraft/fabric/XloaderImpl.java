...
public class XloaderImpl implements Xloader {
    ...
    public static String getModVersion() {
        if (Xloader.isModLoadedSuccess()) {
            return FabricLoader.getInstance().getModContainer("fuvr").get().getMetadata().getVersion()
                .getFriendlyString();
        }
        return "no version";
    }
    ...
    public static Path getJarPath() {
        return FabricLoader.getInstance().getModContainer("fuvr").get().getRootPaths().get(0);
    }
